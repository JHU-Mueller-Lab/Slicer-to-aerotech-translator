...
###################### IMPORT / EXPORT FILES ##############################################################################################################
import_gcode_txt = "NBrown_Helical_Gear_V4.2_gcode.txt"       # upload gcode as txt file (i.e., must include .txt)
export_gcode_txt = "NBrown_Helical_Gear_V4.2_aerotech.txt"    # export gcode as txt file (i.e., must include .txt)
    ...
    with open(export_gcode_txt, type_open) as f:
        ...
        for i in range(len(gcode_list)):
            ...
            if "G1 ;not extruding " in elem:
                ...
                f.write(toggleOFF_1)
                f.write('\n'+elem)
                # gcode_list.insert(i, toggleOFF_1)
                on = False
            elif ';extruding ' in elem and on == False:
                f.write(toggleON_1)
                f.write('\n'+elem)
                # gcode_list.insert(i-1, toggleON)
                on = True
            else:
                f.write('\n'+elem)
    return f
